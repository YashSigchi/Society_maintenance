...
* **Residents:** Anyone living in the society can freely register for a resident account at any time to start raising and tracking their complaints. 
* **Administrators:** To maintain security and governance, new admin accounts cannot be created via the public registration page. A new administrator can only be provisioned by an existing admin through the internal **Admin Management** dashboard.
...
* **🛠️ Complaint Management:** Bulk status updates, priority assignment, and resolution tracking.
...
graph TD
    ...
    API -->|Memory Buffer Stream| Cloudinary
    Cloudinary -->|Secure URL| API
    API -->|Async SMTP| Email
```
